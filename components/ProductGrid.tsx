...
function ProductCard({ product, index, onProductClick, onAddToCart, originalIndex }: {
    product: Product;
    index: number;
    onProductClick: (index: number) => void;
    onAddToCart: (product: Product) => void;
...
export default function ProductGrid({ products, onProductClick, onAddToCart }: ProductGridProps) {
    // Split products into hoodies and pants
    const hoodies = products.filter(p => p.title.toLowerCase().includes('hoodie'));
    const pants = products.filter(p => p.title.toLowerCase().includes('pant'));

    // Get original index for product click handling
    const getOriginalIndex = (product: Product) => products.findIndex(p => p.id === product.id);
    ...
    return (
        <section className="min-h-screen bg-[#0D0D0D] py-32 px-4 md:px-12 relative z-20">
            <div className="max-w-[90rem] mx-auto">
                ...
                {/* HOODIES Section */}
                <div id="desktop-hoodies" className="mb-32 scroll-mt-24">
                    <motion.div
                        initial={{ opacity: 0 }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-32">
                        {hoodies.map((product, index) => (
                            <ProductCard
                                ...
                                originalIndex={getOriginalIndex(product)}
                            />
                        ))}
                    </div>
                </div>
                ...
                {/* PANTS Section */}
                <div id="desktop-pants" className="mb-32 scroll-mt-24">
                    <motion.div
                        initial={{ opacity: 0 }}
                        whileInView={{ opacity: 1 }}
                        transition={{ duration: 0.5 }}
